...
def closer_city(lat, lon, city_a, city_b):
    ...
    x1, x2 = get_lat(city_a), get_lat(city_b)
    y1, y2 = get_lon(city_a), get_lon(city_b)
    a, b = (x1 - lat)**2 + (y1 - lon)**2, (lat - x2)**2 + (lon - y2)**2
    if a==b:
        return 'Stanford'
    else:
...
def dejavu(t, n):
    ...
    def sum(t, cnt, ls):
        if is_leaf(t):
            ls.append(cnt+label(t))
            return 
        for i in branches(t):
            sum(i, cnt+label(t), ls)
    ls = []
    sum(t, 0, ls)
    if n in ls:
        return True
...
def hailstone_tree(n, h):
    ...
    # if h == 0:
    #     return tree(n)
    # branches =  [hailstone_tree(2*n , h - 1)] # [hailstone_tree(n // 2, h - 1)] if n % 2 == 0 else
